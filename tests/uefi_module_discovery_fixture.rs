...
//! `qsp.software.tracker.tracker_obj->maps` dump captured on vmsifter (real Simics
...
/// structure observed in the vmsifter tracker dump (see `src/uefi/mod.rs`'s
/// module doc comment for the real captured example this mirrors).
const PREFIX: &str = "/home/mtarral/tsffs-bmc-bios-poc/bios-x58i/project/workspace/Build/SimicsOpenBoardPkg/BoardX58Ich10/DEBUG_GCC/X64";
...
fn skips_invalid_top_level_entries_without_erroring() -> Result<()> {
    // Confirmed live on vmsifter (a real QSP fuzzing run whose `HARNESS_START`
